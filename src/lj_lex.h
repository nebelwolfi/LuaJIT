...
#define TKDEF(_, __) \
  ...
  __(label, ::) __(number, <number>) __(name, <name>) __(string, <string>) __(lambda, <lambda>)__(lambdaend, `) \
  __(band, &) __(bor, |) __(shl, <) __(shr, >) \
  __(cadd, +=) __(csub, -=) __(cmul, *=) __(cdiv, /=) __(cconcat, ..=) __(cmod, %=) __(cpow, ^=) \
  __(cband, &=) __(cbor, |=) __(cshl, <<=) __(cshr, >>=) \
  __(eof, <eof>)
