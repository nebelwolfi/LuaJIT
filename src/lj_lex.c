...
unsigned int HashFNV1a(const char *name, unsigned int len)
{
...
unsigned int HashIgnoreCaseFNV1a(const char *name, unsigned int len)
{
  unsigned int h = 0x811C9DC5;
  for (unsigned int i = 0; i < len; i++) {
...
unsigned int HashFNV1(const char *name, unsigned int len)
{
...
unsigned int HashIgnoreCaseFNV1(const char *name, unsigned int len)
{
  unsigned int h = 2166136261U;
  for (unsigned int i = 0; i < len; i++) {
    h *= 16777619;
...
unsigned int HashSDBM(const char *name, unsigned int len)
{
  unsigned int h = 0;
  for (unsigned int i = 0; i < len; i++) {
    h = name[i] + (h << 6) + (h << 16) - h;
...
unsigned int HashIgnoreCaseSDBM(const char *name, unsigned int len)
{
  unsigned int h = 0;
  for (unsigned int i = 0; i < len; i++) {
    if (name[i] >= 'A' && name[i] <= 'Z')
...
unsigned int HashELF(const char *name, unsigned int len)
{
  unsigned int h = 0;
  for (unsigned int i = 0; i < len; i++) {
    h = (h << 4) + name[i];
...
unsigned int HashIgnoreCaseELF(const char *name, unsigned int len)
{
  unsigned int h = 0;
  for (unsigned int i = 0; i < len; i++) {
    if (name[i] >= 'A' && name[i] <= 'Z')
...
/* Parse a hash. */
static void lex_hash(LexState *ls, TValue *tv)
{
  LexChar delim = '`';  /* Delimiter is '\'' or '"'. */
  lex_save(ls, delim);
  while (ls->c != delim) {
    switch (ls->c) {
    case LEX_EOF:
  ...
  if (p[0] != '{')
  {
    tv->n = (lua_Number)HashIgnoreCaseFNV1a(p, sbuflen(&ls->sb)-2);
    return;
  } else {
    p++;
    int ignorecase = p[0] != '!';
    ...
    unsigned int(*hash_func)(const char *, unsigned int) =
        casecmp(p[0], 'e') && casecmp(p[1], 'l') && casecmp(p[2], 'f') && p[3] == '}' ? (ignorecase ? HashIgnoreCaseELF : HashELF)
      : casecmp(p[0], 's') && casecmp(p[1], 'd') && casecmp(p[2], 'b') && casecmp(p[3], 'm') && p[4] == '}' ? (ignorecase ? HashIgnoreCaseSDBM : HashSDBM)
      : casecmp(p[0], 'f') && casecmp(p[1], 'n') && casecmp(p[2], 'v') && p[3] == '1' && p[4] == '}' ? (ignorecase ? HashIgnoreCaseFNV1 : HashFNV1)
      : casecmp(p[0], 'f') && casecmp(p[1], 'n') && casecmp(p[2], 'v') && p[3] == '1' && casecmp(p[4], 'a') && p[5] == '}' ? (ignorecase ? HashIgnoreCaseFNV1a : HashFNV1a)
      : 0;
    if (!hash_func)
      lj_lex_error(ls, '`', LJ_ERR_XHASH);
    do {
      p++;
    } while (*p != '}');
    p++;
    tv->n = (lua_Number)hash_func(p, sbuflen(&ls->sb)-(p-ls->sb.b)-1);
    return;
  }
  lj_lex_error(ls, '`', LJ_ERR_XHASH);
}
...
static LexToken lex_scan(LexState *ls, TValue *tv)
{
  ...
  for (;;) {
    ...
    case '&':
      lex_next(ls);
      if (ls->c == '=') { lex_next(ls); return TK_cband; }
      return TK_band;
    case '|':
      lex_next(ls);
      if (ls->c == '=') { lex_next(ls); return TK_cbor; }
      return TK_bor;
    case '<':
      lex_next(ls);
      ...
      if (ls->c == '<') { 
        lex_next(ls);
        if (ls->c == '=') { lex_next(ls); return TK_cshl; }
        return TK_shl;
      }
      return '<';
    case '>':
      ...
      if (ls->c == '>') { 
        lex_next(ls);
        if (ls->c == '=') { lex_next(ls); return TK_cshr; }
        return TK_shr;
      }
      return '>';
    case '~':
...
void lj_lex_error(LexState *ls, LexToken tok, ErrMsg em, ...)
{
  ...
  if (tok == 0) {
    tokstr = NULL;
  } else if (tok == TK_name || tok == TK_string || tok == TK_number || tok == '`') {
    lex_save(ls, '\0');
    tokstr = ls->sb.b;
  } else {
