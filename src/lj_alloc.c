...
{
  ntavm = (PNTAVM)LJ_WIN_GETPROCADDR(NULL, GetModuleHandleA("ntdll.dll"),
...
{
  DWORD olderr = GetLastError();
  void *ptr = LJ_WIN_VALLOC(0, size, MEM_RESERVE|MEM_COMMIT, PAGE_EXECUTE_READWRITE);
  SetLastError(olderr);
...
{
  DWORD olderr = GetLastError();
  void *ptr = LJ_WIN_VALLOC(0, size, MEM_RESERVE|MEM_COMMIT|MEM_TOP_DOWN, PAGE_EXECUTE_READWRITE);
  SetLastError(olderr);
  return ptr ? ptr : MFAIL;
}
