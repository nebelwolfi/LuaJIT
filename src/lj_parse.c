...
LJ_STATIC_ASSERT((int)BC_BANDVV-(int)BC_ADDVV == (int)OPR_BAND-(int)OPR_ADD);
LJ_STATIC_ASSERT((int)BC_BORVV-(int)BC_ADDVV == (int)OPR_BOR-(int)OPR_ADD);
LJ_STATIC_ASSERT((int)BC_BXORVV-(int)BC_ADDVV == (int)OPR_BXOR-(int)OPR_ADD);
LJ_STATIC_ASSERT((int)BC_SHLVV-(int)BC_ADDVV == (int)OPR_SHL-(int)OPR_ADD);
LJ_STATIC_ASSERT((int)BC_SHRVV-(int)BC_ADDVV == (int)OPR_SHR-(int)OPR_ADD);
...
static void fscope_loop_continue(FuncState *fs, BCPos pos)
{
  ...
  lj_assertFS((bl->flags & FSCOPE_LOOP), "expected loop scope");
...
static void parse_lambda_chunk(LexState *ls);

/* Parse body of a function. */
static void parse_lambda_body(LexState *ls, ExpDesc *e, int needself, BCLine line)
{
  FuncState fs, *pfs = ls->fs;
  FuncScope bl;
  GCproto *pt;
  ...
  fs.bclim = pfs->bclim - pfs->pc;
  bcemit_AD(&fs, BC_FUNCF, 0, 0);  /* Placeholder. */
  parse_lambda_chunk(ls);
  if (ls->tok != TK_lambdaend) lex_match(ls, TK_lambdaend, TK_lambda, line);
  pt = fs_finish(ls, (ls->lastline = ls->linenumber));
  pfs->bcbase = ls->bcstack + oldbase;  /* May have been reallocated. */
  pfs->bclim = (BCPos)(ls->sizebcstack - oldbase);
  /* Store new prototype in the constant array of the parent. */
...
static void expr_simple(LexState *ls, ExpDesc *v)
{
  ...
  case TK_lambda:
    lj_lex_next(ls);
    parse_lambda_body(ls, v, 0, ls->linenumber);
    return;
  default:
    expr_primary(ls, v);
    return;
...
static int assign_compound (LexState *ls, LHSVarList *lh, LexToken opType) {
  ...
  switch (opType) {
      ...
      case TK_cband: op = OPR_BAND; break;
      case TK_cbor: op = OPR_BOR; break;
      case TK_cshl: op = OPR_SHL; break;
      case TK_cshr: op = OPR_SHR; break;
...
static void parse_call_assign(LexState *ls)
{
  ...
  } else if (ls->tok >= TK_cadd && ls->tok <= TK_cshr) {
    vl.prev = NULL;
    assign_compound(ls, &vl, ls->tok);
  } else if (ls->tok == ';') {
    /* TK_MINUMINUS should be already managed */
...
/* Check for end of block. */
static int parse_lambda_isend(LexToken tok)
{
  switch (tok) {
  case TK_else: case TK_elseif: case TK_end: case TK_until: case TK_eof: case TK_lambdaend:
    return 1;
  default:
    return 0;
  }
...
static void parse_lambda_chunk(LexState *ls)
{
  int islast = 0;
  synlevel_begin(ls);
  while (!islast && !parse_lambda_isend(ls->tok)) {
    islast = parse_stmt(ls);
    lex_opt(ls, ';');
    lj_assertLS(ls->fs->framesize >= ls->fs->freereg && ls->fs->freereg >= ls->fs->nactvar, "bad regalloc");
    ls->fs->freereg = ls->fs->nactvar;  /* Free registers after each stmt. */
  }
  synlevel_end(ls);
}
