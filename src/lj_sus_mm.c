...
typedef struct {
    ...
    CustomGetModuleHandleFunc getModuleHandle;
...
ResolveApiSetLibraryA(
	...
	PSTRING ResolvedHostLibrary,
	PSTRING ParentName
);

static BOOL
BuildImportTable(PMEMORYMODULE module, const char* DllName)
{
    unsigned char *codeBase = module->codeBase;
    ...
    STRING DllNameStr = {0};
    DllNameStr.Buffer = (char*) DllName;
    DllNameStr.Length = strlen(DllName);
    DllNameStr.MaximumLength = DllNameStr.Length;
    ...
    for (; !IsBadReadPtr(importDesc, sizeof(IMAGE_IMPORT_DESCRIPTOR)) && importDesc->Name; importDesc++) {
        ...
        ResolvedHostLibrary.Length = strlen(ResolvedHostLibrary.Buffer);
        ResolvedHostLibrary.MaximumLength = ResolvedHostLibrary.Length;
        ...
        if (handle == NULL) {
            handle = module->loadLibrary(module->userdata, ResolvedHostLibrary.Buffer);
            if (handle == NULL) {
        ...
        if (tmp == NULL) {
            module->freeLibrary(module->userdata, handle);
        ...
        for (; *thunkRef; thunkRef++, funcRef++) {
            if (IMAGE_SNAP_BY_ORDINAL(*thunkRef)) {
                *funcRef = module->getProcAddress(module->userdata, handle, (LPCSTR)IMAGE_ORDINAL(*thunkRef));
            } else {
                PIMAGE_IMPORT_BY_NAME thunkData = (PIMAGE_IMPORT_BY_NAME) (codeBase + (*thunkRef));
                *funcRef = module->getProcAddress(module->userdata, handle, (LPCSTR)&thunkData->Name);
            }
            if (*funcRef == 0) {
                result = FALSE;
                break;
    ...
    const char* DllName)
{
    ...
    if (!CheckSize(size, sizeof(IMAGE_DOS_HEADER))) {
        printf("CheckSize IMAGE_DOS_HEADER\n");
        return NULL;
    }
    ...
    if (dos_header->e_magic != IMAGE_DOS_SIGNATURE) {
        printf("dos_header->e_magic != IMAGE_DOS_SIGNATURE\n");
        SetLastError(ERROR_BAD_EXE_FORMAT);
        return NULL;
    }

    if (!CheckSize(size, dos_header->e_lfanew + sizeof(IMAGE_NT_HEADERS))) {
        printf("CheckSize IMAGE_NT_HEADERS\n");
        return NULL;
    }
    old_header = (PIMAGE_NT_HEADERS)&((const unsigned char *)(data))[dos_header->e_lfanew];
    if (old_header->Signature != IMAGE_NT_SIGNATURE) {
        printf("old_header->Signature != IMAGE_NT_SIGNATURE\n");
        SetLastError(ERROR_BAD_EXE_FORMAT);
        return NULL;
    }

    if (old_header->FileHeader.Machine != HOST_MACHINE) {
        printf("old_header->FileHeader.Machine != HOST_MACHINE %x\n", old_header->FileHeader.Machine);
        SetLastError(ERROR_BAD_EXE_FORMAT);
        return NULL;
    }
    ...
    if (old_header->OptionalHeader.SectionAlignment & 1) {
        ...
        printf("old_header->OptionalHeader.SectionAlignment & 1\n");
        SetLastError(ERROR_BAD_EXE_FORMAT);
        return NULL;
    }
    ...
    if (alignedImageSize != AlignValueUp(lastSectionEnd, sysInfo.dwPageSize)) {
        printf("alignedImageSize != AlignValueUp(lastSectionEnd, sysInfo.dwPageSize)\n");
        SetLastError(ERROR_BAD_EXE_FORMAT);
        return NULL;
    }
    ...
    result->getModuleHandle = getModuleHandle;
    ...
    if (!BuildImportTable(result, DllName)) {
        goto error;
    }
